...
export default function SosConsole() {
  ...
  async function resolve(id: string) {
    const resolution = prompt('Resolution note (mandatory):');
    if (!resolution) return;
    await api(`/v1/safety/sos/${id}/resolve`, { method: 'POST', body: JSON.stringify({ resolution }) }).then(load).catch(e => alert(e.message));
  }
