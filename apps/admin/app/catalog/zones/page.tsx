...
export default function Zones() {
  ...
  async function addZone(cityId: string) {
    ...
    const center = prompt('Center "lat,lng" (a ~4km square zone is created around it):');
    if (!center) return;
  ...
  async function addHub(cityId: string, zoneId: string) {
    const name = prompt('Hub name:'); if (!name) return;
    const center = prompt('Hub "lat,lng":'); if (!center) return;
    const [lat, lng] = center.split(',').map(Number);
