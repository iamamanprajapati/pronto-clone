...
export default function Workers() {
  ...
  useLiveData(() => api<{ workers: any[] }>('/v1/admin/workers').then(r => {
    ...
    <div>
      ...
      <div className="tableWrap">
        <table>
          ...
          <tbody>
            {workers.map(w => (
              <tr key={w.id} className="click" onClick={() => navigate(`/workforce/workers/${w.id}`)}>
                <td>{w.user.name ?? '—'}</td>
                <td>{w.user.phone}</td>
            ...
            {workers.length === 0 && <tr><td colSpan={9} className="muted">No workers found</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
