...
export default function Payouts() {
  ...
  const load = () => api<{ runs: any[] }>('/v1/earnings/payout-runs')
  ...
  async function createRun() {
    const cycleLabel = await modalPrompt('Cycle label (e.g. "Week 28 · Jul 6–12"):');
    if (!cycleLabel) return;
    await api('/v1/earnings/payout-runs', { method: 'POST', body: JSON.stringify({ cycleLabel }) })
      .then(load)
      .catch(async e => await modalAlert(e.message));
  }
